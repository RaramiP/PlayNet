...
class SteamScraper:
    ...
    def get_all_app_ids(self):
        """Fetches the list of GAMES only (no DLCs) using the new API."""
    ...
    def run(self, limit=1000):
        all_apps = self.get_all_app_ids()
        
        # Safety: If API failed to get list, stop
        if not all_apps:
        ...
        for app in all_apps:
            ...
            # rate Limiting because otherwise api will think it is a bot spamming
            time.sleep(1.5) 
